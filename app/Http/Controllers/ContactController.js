...
class ContactController {
    ...
    * addAction(request, response) {
        ...
        if (contactData.profileImage !== null) {
            ...
            if (profileImage.clientSize() > 0 && !profileImage.validate()) {
                yield request
                    .withAll()
                    .andWith({errors: [{message: profileImage.errors()}]})
                    .flash()
        ...
        // the contact wont be stored, but if it does not, this is the right time to create the image object for it.
        yield contact.save()
        ...
        if (contactData.emails !== null) {
            if (contactData.emails.indexOf('#') !== -1) {
                ...
                const emails = contactData.emails.split('#');

                // Iterate through every email submitted with its type:
                for (let typeMail in emails) {
                ...
                let pieces = contactData.emails.split('-')
        ...
        if (contactData.tnums !== null) {
            if (contactData.tnums.indexOf('#') !== -1) {
                // Then we have to split this thing apart
                const tnums = contactData.tnums.split('#');

                for (let typeNum in tnums) {
                ...
                let pieces = contactData.tnums.split('-')
        ...
        if (contactData.addresses !== null) {
            if (contactData.addresses.indexOf('#') !== -1) {
                // Then we have to split this thing apart
                const addresses = contactData.addresses.split('#');

                for (let typeAddress in addresses) {
                ...
                let pieces = contactData.addresses.split('-')
        ...
        // { _csrf: 'AwgdyUfD-yz5jMtmJwlecMHmNhNoBi_e8V9A',
        //     firstname: 'Attila',
        //     surname: 'Tóth',
        //     nickname: 'atesztoth',
        //     profileImage: 'ncie.jpg',
        //     emails: 'magán-muchcoffe@gmail.com#céges-atoth@loginet.hu',
        //     tnums: 'magán-+36707011006#céges-06707011006',
        //     addresses: 'pazony-4531 Nyírpazony Kabalási út 57#otthon-4562 Vaja, Tompa Mihály utca 10',
        //     comment: 'Yolo swag' }

    }


    static removeAccentsWhatever(str) {
